...
This document describes the release workflow for Portfolio following industry standards based on GitFlow with automated releases.
...
1. Extracts version from branch name (`release/v1.0.0` → `v1.0.0`)
...
On the `release/1.0.0` branch, perform the following tasks:
...
# Hotfix
git checkout main && git pull
git checkout -b hotfix/1.0.1 && git push origin hotfix/1.0.1
gh pr create --base main --head hotfix/1.0.1
# Al mergear: auto-release → deploy
