...
class Portfolio extends React.Component {
      ...
      return items.map((value, index) => {
        if (
          ...
          if (value.content.frontmatter.image) {
            return (
                <a key={index} href={value.content.frontmatter.item_url} target="_blank">
                  <div
                  ...
                  <Tilt className="Tilt" options={{ scale: 1, max: 50 }}>
                    <div className="overlay">
                      <span className="title">
                        {value.content.frontmatter.title} <br/><span className="description">{value.content.frontmatter.description}</span></span>
                    </div>
                  </Tilt>
                </AnimationContainer>
              </div>
  ...
  changeCategory(category) {
    ...
    let col = total > 6 ? 4 : total > 4 ? 3 : total >= 3 ? 3 : total > 1 ? 2 : 1
    console.log(total)
