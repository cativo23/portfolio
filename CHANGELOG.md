...
### Added
- **`/now` page now covers life outside of code.** Added TCGVault, a new side project tracking Pokémon TCG card values against live market pricing, to the side-projects list. Added a new "Off The Clock" section covering Pokémon TCG collecting, playing through the mainline Pokémon games, modding a Civic Type R FK8, and current reading.
...
### Removed
- **Dead `/api/profile` BFF route** — removed the server route and its test. It proxied to the backend's now-removed public profile endpoint but was never called by any page or component; the CV is rendered from hardcoded data. (#134)
